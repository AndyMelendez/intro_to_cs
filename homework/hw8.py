...
def adjoin_set2(s, v):
    """Return a set containing all elements of s and element v.
        ...
        """
    if empty(s):
        return Rlist(v, Rlist.empty)
    if set_contains2(s, v):
        return s
    g1 = s.first
    if g1 < v:
        return Rlist(g1, adjoin_set2(s.rest,v))
    if v < g1:
        return Rlist(v, s)
...
def tree_to_ordered_sequence(s):
   ...
   if s is None:
    ...
    union = union_set2(tree_to_ordered_sequence(s.left), tree_to_ordered_sequence(s.right))
    adjoin = adjoin_set3(union, s.entry)
    return adjoin
