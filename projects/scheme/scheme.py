...
def do_if_form(vals, env):
    ...
    check_form(vals, 3, 3)
    predicate, consequent, alternative = scheme_eval(vals[0], env), vals[1], vals[2]
    if scheme_true(predicate): return consequent
    elif not scheme_true(predicate): return alternative
...
def do_and_form(vals, env):
    ...
    for i in range(len(vals) - 1):
        if scheme_false(scheme_eval(vals[i], env)): return False
...
def do_or_form(vals, env):
    ...
    for i in range(len(vals) - 1):
        if scheme_true(scheme_eval(vals[i], env)): return vals[i]
    return vals[len(vals) - 1]
